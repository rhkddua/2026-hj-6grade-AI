...
import {
  ...
  initialActivities,
  restoreActivities,
  lessonTwoRequirements,
  type LessonTwoActivities,
} from '@/lib/lesson-two';
...
export default function LessonTwoPage() {
  ...
  useEffect(() => {
    ...
    let active = true;
    setLoadError(false);
    void loadLessonProgress(2, true)
      .then((data) => {
        if (!active) return;
        if (data) {
  ...
  function update(patch: Partial<LessonTwoActivities>) {
    dirty();
    setActivities((a) => ({ ...a, ...patch }));
  }
  ...
  if (loadError)
    return (
      <main className="mx-auto max-w-xl space-y-5 p-8">
        ...
        <p>
          기존 기록을 보호하기 위해 불러온 뒤 활동을 시작해요. 인터넷 연결을
          확인해 주세요.
        </p>
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto grid max-w-6xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[220px_minmax(0,1fr)] lg:py-9">
        <aside>
          <div className="rounded-3xl border bg-card p-5 shadow-sm lg:sticky lg:top-24">
            ...
            <p className="mt-5 text-sm leading-6 text-muted-foreground">
              목표: 세 가지 코딩 방법의 차이와 공통점을 설명할 수 있어요.
            </p>
          </div>
        </aside>
        <section className="min-w-0 rounded-3xl border bg-card p-5 shadow-sm sm:p-8">
          ...
          />
          {step === 5 && (
            <ReflectionBoardActions lessonNo={2} reflection={reflection} />
          )}
          {step === 5 && (
            <div className="mt-6 space-y-4">
              <div className="rounded-2xl bg-secondary/60 p-5">
                ...
                <ul className="mt-3 space-y-2">
                  {[
                    ...
                    <li key={s} className="flex gap-2 leading-7">
                      <span aria-label={requirements[i] ? '완료' : '아직'}>
                        {requirements[i] ? '✓' : '○'}
                      </span>
                      {s}
                    </li>
              ...
              {completed && (
                ...
                >
                  <p className="text-xl font-black">2차시 완료!</p>
                  <p className="mt-2 leading-7">
                    비교표와 성찰을 저장했어요. 다시 들어와서 내가 쓴 내용을
                    확인할 수 있어요.
                  </p>
                </div>
              )}
              ...
              >
                <Save />
                {completed ? '완료 기록 저장됨' : '2차시 완료하고 저장'}
              </Button>
            </div>
          )}
