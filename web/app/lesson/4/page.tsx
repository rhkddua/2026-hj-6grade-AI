...
import {
  initialActivities,
  lessonFourRequirements,
  restoreActivities,
  questions,
  stages,
  type LessonFourActivities,
} from '@/lib/lesson-four';
import { LessonFourContent } from './content';
...
export default function LessonFourPage() {
  ...
  const requirements = lessonFourRequirements(activities, reflection);
  ...
  useEffect(() => {
    ...
    let active = true;
    setLoadError(false);
    void loadLessonProgress(4, true)
      .then((data) => {
        if (!active) return;
        if (data) {
  ...
  function update(patch: Partial<LessonFourActivities>) {
    dirty();
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto grid max-w-6xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[220px_minmax(0,1fr)] lg:py-9">
        <aside>
          <div className="rounded-3xl border bg-card p-5 shadow-sm lg:sticky lg:top-24">
            ...
            <p className="mt-5 text-sm leading-6 text-muted-foreground">
              목표: 목표·상황·조건·결과 형식을 담아 안전하게 지시하고, 결과를
              보며 다시 요청할 수 있어요.
            </p>
          </div>
        </aside>
        <section className="min-w-0 rounded-3xl border bg-card p-5 shadow-sm sm:p-8">
          ...
          />
          {step === 5 && (
            <ReflectionBoardActions lessonNo={4} reflection={reflection} />
          )}
          {step === 5 && (
            <div className="mt-6 space-y-4">
              ...
              {completed && (
                ...
                >
                  <p className="text-xl font-black">4차시 완료!</p>
                  <p className="mt-2 leading-7">
                    AI에게 생각을 잘 전하고, 결과를 확인하며 고치는 방법을
                    배웠어요.
                  </p>
                </div>
              )}
              ...
              >
                <Save />
                {completed ? '완료 기록 저장됨' : '4차시 완료하고 저장'}
              </Button>
            </div>
          )}
