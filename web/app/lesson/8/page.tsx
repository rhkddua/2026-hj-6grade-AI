...
export default function LessonEightPage() {
  ...
  useEffect(() => {
    if (!userId) return;
    let active = true;
    void loadLessonProgress(8, true)
      .then((data) => {
        if (!active) return;
        if (data) {
  ...
  const labels = [
    '두 기능 연결 의미 확인',
    '첫 기능·전달 값·두 번째 기능 흐름 확인',
    '안전한 두 기능 제작 지시 확인',
    '세 가지 상황 테스트와 수정 계획 확인',
    '퀴즈 3문항 모두 정답 확인',
    '성찰 10자 이상 작성',
  ];
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto grid max-w-6xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[220px_minmax(0,1fr)] lg:py-9">
        <aside>
          <div className="rounded-3xl border bg-card p-5 shadow-sm lg:sticky lg:top-24">
            ...
            <p className="mt-5 text-sm leading-6 text-muted-foreground">
              목표: 첫 기능의 결과를 다음 기능에 연결하고, 여러 상황에서 직접
              테스트할 수 있어요.
            </p>
          </div>
        </aside>
        <section className="min-w-0 rounded-3xl border bg-card p-5 shadow-sm sm:p-8">
          ...
          />
          {step === 5 && (
            <ReflectionBoardActions lessonNo={8} reflection={reflection} />
          )}
          {step === 5 && (
            <div className="mt-6 space-y-4">
              ...
              {completed && (
                <output className="block rounded-2xl border-2 border-primary p-5">
                  <span className="block text-xl font-black">8차시 완료!</span>
                  <span className="mt-2 block leading-7">
                    두 기능을 하나의 흐름으로 연결하고 여러 상황에서 시험하는
                    방법을 배웠어요.
                  </span>
                </output>
              )}
              ...
              >
                <Save />
                {completed ? '완료 기록 저장됨' : '8차시 완료하고 저장'}
              </Button>
            </div>
          )}
