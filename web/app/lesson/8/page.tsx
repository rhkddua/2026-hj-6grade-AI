...
import {
  initialActivities,
  lessonEightRequirements,
  questions,
  restoreActivities,
  stages,
  type LessonEightActivities,
} from '@/lib/lesson-eight';
import { LessonEightContent } from './content';
...
export default function LessonEightPage() {
  ...
  const requirements = lessonEightRequirements(activities, reflection);
  ...
  useEffect(() => {
    if (!userId) return;
    let active = true;
    void loadLessonProgress(8, true).then(data => {
      if (!active) return;
      if (data) {
  ...
  function update(patch: Partial<LessonEightActivities>) {
    dirty();
  ...
  return <div className="min-h-screen bg-background">
    ...
    <main className="mx-auto grid max-w-6xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[220px_minmax(0,1fr)] lg:py-9">
      <aside><div className="rounded-3xl border bg-card p-5 shadow-sm lg:sticky lg:top-24"><Badge>8차시 · 40분</Badge><h1 className="mt-3 text-xl font-black leading-8">두 기능 앱<br />만들기</h1><Progress value={completed ? 100 : (step - 1) * 20} className="mt-5"><ProgressLabel>학습 진행</ProgressLabel><ProgressValue /></Progress><nav aria-label="8차시 학습 단계"><ol className="mt-5 space-y-2">{stages.map((title, index) => <li key={title}><button onClick={() => go(index + 1)} aria-current={step === index + 1 ? 'step' : undefined} className={`w-full rounded-xl px-3 py-3 text-left text-sm font-bold focus-visible:ring-2 focus-visible:ring-primary ${step === index + 1 ? 'bg-secondary text-secondary-foreground' : 'hover:bg-muted'}`}>{index + 1}. {title}</button></li>)}</ol></nav><p className="mt-5 text-sm leading-6 text-muted-foreground">목표: 첫 기능의 결과를 다음 기능에 연결하고, 여러 상황에서 직접 테스트할 수 있어요.</p></div></aside>
      <section className="min-w-0 rounded-3xl border bg-card p-5 shadow-sm sm:p-8"><p className="text-sm font-bold text-primary">STEP {step} / 5</p><h2 ref={heading} tabIndex={-1} className="mt-2 text-2xl font-black leading-tight outline-none sm:text-3xl">{titles[step - 1]}</h2><LessonEightContent step={step} activities={activities} update={update} reflection={reflection} setReflection={value => { dirty(); setReflection(value); }} />
        {step === 5 && <div className="mt-6 space-y-4"><div className="rounded-2xl bg-secondary/60 p-5"><h3 className="font-black">완료 전 확인</h3><ul className="mt-3 space-y-2 leading-7">{labels.map((label, index) => <li key={label}>{requirements[index] ? '✓' : '○'} {label}</li>)}</ul></div>{completed && <output className="block rounded-2xl border-2 border-primary p-5"><span className="block text-xl font-black">8차시 완료!</span><span className="mt-2 block leading-7">두 기능을 하나의 흐름으로 연결하고 여러 상황에서 시험하는 방법을 배웠어요.</span></output>}<Button onClick={() => void finish()} disabled={!requirements.every(Boolean) || saving || completed}><Save />{completed ? '완료 기록 저장됨' : '8차시 완료하고 저장'}</Button></div>}
