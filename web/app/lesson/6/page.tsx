...
import {
  initialActivities,
  lessonSixRequirements,
  restoreActivities,
  questions,
  stages,
  type LessonSixActivities,
} from '@/lib/lesson-six';
import { LessonSixContent } from './content';
...
export default function LessonSixPage() {
  ...
  const requirements = lessonSixRequirements(activities, reflection);
  ...
  useEffect(() => {
    ...
    let active = true;
    setLoadError(false);
    void loadLessonProgress(6, true)
      .then((data) => {
        if (!active) return;
        if (data) {
  ...
  function update(patch: Partial<LessonSixActivities>) {
    dirty();
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto grid max-w-6xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[220px_minmax(0,1fr)] lg:py-9">
        <aside>
          <div className="rounded-3xl border bg-card p-5 shadow-sm lg:sticky lg:top-24">
            ...
            <p className="mt-5 text-sm leading-6 text-muted-foreground">
              목표: 한 가지 기능의 입력·동작·결과를 정하고, 안전하게 만들고
              테스트할 수 있어요.
            </p>
          </div>
        </aside>
        <section className="min-w-0 rounded-3xl border bg-card p-5 shadow-sm sm:p-8">
          ...
          />
          {step === 5 && (
            <ReflectionBoardActions lessonNo={6} reflection={reflection} />
          )}
          {step === 5 && (
            <div className="mt-6 space-y-4">
              ...
              {completed && (
                ...
                >
                  <p className="text-xl font-black">6차시 완료!</p>
                  <p className="mt-2 leading-7">
                    한 가지 중요한 기능을 먼저 만들고, 직접 테스트하며 개선하는
                    방법을 배웠어요.
                  </p>
                </div>
              )}
              ...
              >
                <Save />
                {completed ? '완료 기록 저장됨' : '6차시 완료하고 저장'}
              </Button>
            </div>
          )}
