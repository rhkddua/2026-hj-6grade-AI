...
import {
  initialActivities,
  lessonThreeRequirements,
  restoreActivities,
  questions,
  stages,
  type LessonThreeActivities,
} from '@/lib/lesson-three';
import { LessonThreeContent } from './content';
...
export default function LessonThreePage() {
  ...
  const requirements = lessonThreeRequirements(activities, reflection);
  ...
  useEffect(() => {
    ...
    let active = true;
    setLoadError(false);
    void loadLessonProgress(3, true)
      .then((data) => {
        if (!active) return;
        if (data) {
  ...
  function update(patch: Partial<LessonThreeActivities>) {
    dirty();
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto grid max-w-6xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[220px_minmax(0,1fr)] lg:py-9">
        <aside>
          <div className="rounded-3xl border bg-card p-5 shadow-sm lg:sticky lg:top-24">
            ...
            <p className="mt-5 text-sm leading-6 text-muted-foreground">
              목표: AI의 강점과 한계를 알고, 결과를 확인하며 안전하게 사용할 수
              있어요.
            </p>
          </div>
        </aside>
        <section className="min-w-0 rounded-3xl border bg-card p-5 shadow-sm sm:p-8">
          ...
          />
          {step === 5 && (
            <ReflectionBoardActions lessonNo={3} reflection={reflection} />
          )}
          {step === 5 && (
            <div className="mt-6 space-y-4">
              ...
              {completed && (
                ...
                >
                  <p className="text-xl font-black">3차시 완료!</p>
                  <p className="mt-2 leading-7">
                    AI를 안전하게 사용하는 방법을 배웠어요.
                  </p>
                </div>
              )}
              <Button
              ...
              >
                <Save />
                {completed ? '완료 기록 저장됨' : '3차시 완료하고 저장'}
              </Button>
            </div>
          )}
