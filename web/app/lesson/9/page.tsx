...
export default function LessonNinePage() {
  ...
  useEffect(() => {
    if (!userId) return;
    let active = true;
    void loadLessonProgress(9, true)
      .then((data) => {
        if (!active) return;
        if (data) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto grid max-w-6xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[220px_minmax(0,1fr)] lg:py-9">
        <aside>
          <div className="rounded-3xl border bg-card p-5 shadow-sm lg:sticky lg:top-24">
            ...
            <p className="mt-5 text-sm leading-6 text-muted-foreground">
              목표: 생활 속 문제를 찾고, 나에게 필요한 두 기능 앱의 제작·테스트
              계획을 완성할 수 있어요.
            </p>
          </div>
        </aside>
        <section className="min-w-0 rounded-3xl border bg-card p-5 shadow-sm sm:p-8">
          ...
          />
          {step === 5 && (
            <ReflectionBoardActions lessonNo={9} reflection={reflection} />
          )}
          {step === 5 && (
            <div className="mt-6 space-y-4">
              ...
              {completed && (
                <output className="block rounded-2xl border-2 border-primary p-5">
                  <span className="block text-xl font-black">9차시 완료!</span>
                  <span className="mt-2 block leading-7">
                    생활 속 문제에서 시작해 나에게 필요한 앱의 계획과 개선
                    방법을 완성했어요.
                  </span>
                </output>
              )}
              ...
              >
                <Save />
                {completed ? '완료 기록 저장됨' : '9차시 완료하고 저장'}
              </Button>
            </div>
          )}
