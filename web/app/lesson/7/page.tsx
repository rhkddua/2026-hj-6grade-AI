...
import {
  initialActivities,
  lessonSevenRequirements,
  questions,
  restoreActivities,
  stages,
  type LessonSevenActivities,
} from '@/lib/lesson-seven';
import { LessonSevenContent } from './content';
export default function LessonSevenPage() {
  ...
  const [step, setStep] = useState(1),
    ...
    [status, setStatus] = useState('학습 기록을 불러오는 중…'),
  ...
  const requirements = lessonSevenRequirements(activities, reflection),
  ...
  useEffect(() => {
    ...
    let active = true;
    setLoadError(false);
    void loadLessonProgress(7, true)
      .then((data) => {
        if (!active) return;
        if (data) {
  ...
  function update(patch: Partial<LessonSevenActivities>) {
    dirty();
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto grid max-w-6xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[220px_minmax(0,1fr)] lg:py-9">
        <aside>
          <div className="rounded-3xl border bg-card p-5 shadow-sm lg:sticky lg:top-24">
            ...
            <p className="mt-5 text-sm leading-6 text-muted-foreground">
              목표: 사용자의 행동과 앱의 결과를 연결해 안전한 기능을 설계할 수
              있어요.
            </p>
          </div>
        </aside>
        <section className="min-w-0 rounded-3xl border bg-card p-5 shadow-sm sm:p-8">
          ...
          />
          {step === 5 && (
            <ReflectionBoardActions lessonNo={7} reflection={reflection} />
          )}
          {step === 5 && (
            <div className="mt-6 space-y-4">
              ...
              {completed && (
                ...
                >
                  <p className="text-xl font-black">7차시 완료!</p>
                  <p className="mt-2 leading-7">
                    사용자가 하는 일과 앱의 결과를 연결해 기능을 설계하는 방법을
                    배웠어요.
                  </p>
                </div>
              )}
              ...
              >
                <Save />
                {completed ? '완료 기록 저장됨' : '7차시 완료하고 저장'}
              </Button>
            </div>
          )}
