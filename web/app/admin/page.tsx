      ...
      <div className="mx-auto grid max-w-[1600px] lg:grid-cols-[240px_minmax(0,1fr)]">
        <aside className="hidden min-h-[calc(100vh-72px)] border-r border-slate-200 bg-white px-4 py-6 lg:block">
          <nav aria-label="관리자 메뉴" className="space-y-1.5">
            <a href="#dashboard" className="flex items-center gap-3 rounded-xl bg-slate-950 px-3 py-3 text-sm font-bold text-white"><LayoutDashboard className="size-4.5" />대시보드</a>
            <a href="#students" className="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-bold text-slate-600 hover:bg-slate-100"><Users className="size-4.5" />학생 현황</a>
            <a href="#lessons" className="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-bold text-slate-600 hover:bg-slate-100"><BookOpenCheck className="size-4.5" />차시별 진도</a>
          </nav>
        ...
        <main id="dashboard" className="min-w-0 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
          <section className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div><p className="text-sm font-bold text-teal-700">운영 현황</p><h1 className="mt-1 font-heading text-3xl font-black tracking-[-0.04em] sm:text-4xl">관리자 대시보드</h1><p className="mt-2 text-base text-slate-600">등록 학생과 1~{ACTIVE_LESSONS}차시 학습 기록을 확인합니다.</p></div>
            <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => downloadCsv(filteredStudents)} disabled={filteredStudents.length === 0}><Download data-icon="inline-start" />CSV 내려받기</Button><Button onClick={() => void refresh()} disabled={loading} className="bg-slate-950 text-white hover:bg-slate-800"><RefreshCw className={loading ? 'animate-spin' : ''} data-icon="inline-start" />새로고침</Button></div>
          </section>
          ...
          <section id="students" className="mt-5 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          ...
          <section id="lessons" className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
