  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-20 border-b border-border/80 bg-background/92 backdrop-blur-xl">
        <div className="mx-auto flex h-18 max-w-[1440px] items-center justify-between px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-heading text-lg font-extrabold tracking-[-0.03em]">
                AI 코딩 교실
              </p>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="hidden text-right sm:block">
              <p className="text-sm font-bold">
                6학년 {profile?.class_no ?? '-'}반 ·{' '}
                {profile?.student_no ?? '-'}번
              </p>
              <p className="text-xs text-muted-foreground">{studentName}</p>
            </div>
            <div className="grid size-10 place-items-center rounded-full bg-secondary font-extrabold text-secondary-foreground">
              {initial}
            </div>
            <Button
              variant="ghost"
      ...
      <div className="mx-auto grid max-w-[1440px] grid-cols-1 lg:grid-cols-[220px_minmax(0,1fr)]">
        <aside className="hidden min-h-[calc(100vh-72px)] border-r border-border/70 px-4 py-6 lg:block">
          <nav aria-label="학생 메뉴" className="space-y-1.5">
            <a className="nav-item nav-item-active" href="#top">
          ...
          <div className="mt-8 rounded-2xl border border-amber-200/80 bg-amber-50 p-4 text-amber-950">
            ...
            <p className="text-sm leading-6 text-amber-900/80">
              AI의 답은 꼭 직접 확인하고, 개인정보는 입력하지 않아요.
            </p>
          </div>
        </aside>
        ...
        <main id="top" className="min-w-0 px-4 py-6 sm:px-6 lg:px-10 lg:py-8">
          <section className="mb-7 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            <div>
              <Badge className="mb-3 h-7 bg-teal-100 px-3 text-teal-800 hover:bg-teal-100">
          ...
          <section className="current-lesson-card relative overflow-hidden rounded-[28px] p-6 sm:p-8">
            ...
            <div className="lesson-orbit" aria-hidden="true">
              <div className="grid size-18 place-items-center rounded-3xl bg-white/16 text-white shadow-xl backdrop-blur">
          ...
          <section id="lessons" className="mt-9">
            <div className="mb-4 flex items-end justify-between gap-4">
              <div>
                <h2 className="font-heading text-xl font-black tracking-[-0.02em] sm:text-2xl">
                  10번의 AI 코딩 모험
                </h2>
                <p className="mt-1 text-sm text-muted-foreground">
                  차근차근 배우고, 마지막에는 나만의 앱을 완성해요.
                </p>
              </div>
          ...
          <section className="mt-8 grid gap-4 md:grid-cols-3" id="works">
            ...
            >
              <div className="flex items-start gap-4">
                <div className="grid size-11 shrink-0 place-items-center rounded-2xl bg-teal-100 text-teal-700">
      ...
      <nav
        ...
        className="fixed inset-x-3 bottom-3 z-30 grid grid-cols-5 rounded-2xl border bg-card/95 p-1.5 shadow-xl backdrop-blur lg:hidden"
      >
        <a className="mobile-nav-item text-primary" href="#top">
          <Home className="size-5" />
          <span>홈</span>
        </a>
        <a className="mobile-nav-item" href="#lessons">
