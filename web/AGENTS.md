...
다음 세션의 최우선 작업은 3차시 **「AI는 무엇을 잘하고 못할까?」** 구현이다. 구체적인 콘텐츠·완료 조건·검증 목록은 저장소 루트의 `SESSION_HANDOFF.md`를 따른다. 교사 대시보드, 최고관리자 UI, 홈 전체 진도 실데이터화는 사용자가 별도로 요청하지 않으면 3차시 범위에 섞지 않는다.
