...
2. 실제 학생 세션으로 화면, 입력, 저장, 새로고침 복원, 완료/실패 흐름을 확인해야 한다. 테스트 계정은 만들지 않았다.
...
4. 로그인 요청을 사용자에게 보냈으나 아직 완료되지 않았다. 실제 학생 데이터를 임의로 수정하거나 테스트 계정을 생성하지 않았다. 공개 사이트 로그인 화면을 열어 두었다.
